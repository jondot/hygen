...
describe('render', function () {
  ftest('empty', { app: { action: { 'empty.ejs.t': fixture('empty.ejs.t') } } }, function () {
    var res = render({ actionfolder: 'app/action' });
    expect(res[0].file).toMatch(/empty/);
    res[0].file = 'empty.ejs.t';
    expect(res[0].body).toEqual('');
  });
  ftest('full', { app: { action: { 'full.ejs.t': fixture('full.ejs.t') } } }, function () {
    var res = render({
      bill: 17,
      name: 'someone',
  ...
  ftest('capitalized', { app: { action: { 'capitalized.ejs.t': fixture('capitalized.ejs.t') } } }, function () {
    var res = render({ name: 'someone', actionfolder: 'app/action' });
    expect(res[0].file).toMatch(/capitalized/);
    res[0].file = 'capitalize.ejs.t';
    expect(res[0].body).toMatch(/someone and Someone/);
  });

  ftest('capitalized with default locals', { app: { action: { 'capitalized.ejs.t': fixture('capitalized.ejs.t') } } }, function () {
    var res = render({ actionfolder: 'app/action' });
    expect(res[0].file).toMatch(/capitalized/);
    res[0].file = 'capitalize.ejs.t';
  ...
  ftest('render should do all files in an action folder ', {
    app: {
      action: {
    ...
    var res = render({ bill: 17, actionfolder: 'app/action' });
    expect(res.length).toEqual(2);
    expect(res[0].file).toMatch(/capitalized/);
    expect(res[1].file).toMatch(/full/);
  });

  ftest('render with subaction should filter only to that subaction', {
    app: {
      action: {
        'capitalized.ejs.t': fixture('capitalized.ejs.t'),
        'full.ejs.t': fixture('full.ejs.t')
